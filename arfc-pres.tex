...
\begin{frame}
\frametitle{Technical Gaps}
\begin{itemize}
    \item Multiplication factor
    \item Non-normal operation
    \begin{itemize}
...
\begin{frame}
\frametitle{Future Work by authors}
\begin{itemize}
...
\begin{itemize}
  ...
  core designs with a much longer lifetime of 800 EFPD
  \item Effect of local moderation on used fuel composition and flux
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Accident Scenarios}
...
\begin{itemize}
    ...
    \item Hydrogen desorption possible in higher temperatures (in-pile tests only consider up to 700$^\cdot$C)
\end{itemize}
\end{frame}
...
\begin{enumerate}
    \item \hfh in higher temperature than 700$^\cdot$C and observe hydrogen desorption with temperature
    \item Calculate \hfh control rod in-core hydrogen desorption in accident conditions
    \item Calculate change in control rod worth with hydrogen desorption
...
Reactor Shutdown System - passively released due to curie point effect by abnormal rising of coolant temperature.
Passive safety -> thermal expansion of absorbing liquid
gas expansion leads to more neutron leakage
